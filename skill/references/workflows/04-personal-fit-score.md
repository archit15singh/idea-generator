...
- `templates/founder-profile.md`: your stack, markets you can name players in, distribution channels, cold-reachable buyer personas, what you'd do for free. Load once per agent session.
- `wedges` rows per startup (from stage 03)

## Do (one row per startup; one wedge-fit row per wedge later)

Score 0-10 across 8 axes (`references/design/personal-fit-score.md`):
...
`total` = sum 0-80. Store per-axis values, not just the total. Shape matters more than score. A 60 from `10/10/2/8/10/10/2/8` is a build-fast-sell-it-yourself bet, very different from a 60 spread as `5/5/6/6/6/6/6/20-invalid` which signals a data-entry problem, not a strategy.
...
- `references/design/personal-fit-score.md`
- `references/design/personalisation-and-founder-history.md`