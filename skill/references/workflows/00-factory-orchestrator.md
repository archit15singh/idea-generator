...
- `references/design/personalisation-and-founder-history.md`: the constrained market pool (3 ICP clusters) + your founder-history profile (stack, cold-reachable buyers, what-you'd-do-for-free). Unlocks personal-fit scoring in stage 04.
- `references/design/problem-graph.md`: controlled vocabulary of problem-IDs + fixed edge set. Required before any promotion in stage 07.
...
2. `02-descend-recursive.md`: recursive framework L1 to L10 per startup. L5 is the only wedge-generating level; spend disproportionate time there.
...
After 8 weeks of agent runtime, one wedge must have 3+ prospect replies indicating real pain. If not reached, STOP the factory. Do not iterate on outreach copy. Re-tune founder history (`references/design/personalisation-and-founder-history.md`), then re-descend (02) and re-wedge (03) for affected startups.

## Honour rules

- Validation before build. (Structural problem #1, `references/design/structural-problems.md`.)
- MVPs are surveys, not products. Instrumented from line one. Uninstrumented MVPs produce no data; do not rebuild, re-extract.
...
- Reject any wedge with no evidence citation in `startup_competitive` or `startup_customer`. No evidence means not selected.

## Refs

- All raw design files: `references/design/*.md`. The workflow files consolidate them into prompts; the raw files hold the why behind each call when something feels under-specified.