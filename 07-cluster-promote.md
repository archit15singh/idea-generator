# 07 / Cluster + promote (Pattern Library + Problem Graph)
...
- Normalize before grouping using the controlled vocabulary in `raw/problem-graph.md` (resolve "AI memory" / "context retention" / "session memory" to one canonical problem-id).
...
For each promoted pattern (raw `pattern-library.md`), write a mini-spec:
...
- For each wedge that graduated stage 06, write a `solves` edge from the wedge → problem.
- For each interview finding from 6.4, write a `suffers-from` edge from the customer-complaint node → the problem.

## 7.4 Retire

A pattern whose `last_growth_rate ≤ 0` for ≥30 days → stamp `retired_at`. Saturated patterns are noise; keeping them pollutes the pattern library. Don't collect patterns you "should build" — collect patterns the data says recur.
...
- raw `pattern-library.md`, `raw/problem-graph.md`, `raw/analysis.md`