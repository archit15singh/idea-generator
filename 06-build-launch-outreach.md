...
Runs **only on wedges that graduated from stage 05**. The MVP is a **survey of demand**, not a product. Uninstrumented MVP = useless row.
...
- raw `structural-problems.md` — MVP-as-survey argument
- raw `factory.md` — the per-wedge stages
- raw `analysis.md` — the open questions on failure-mode and instrumentation