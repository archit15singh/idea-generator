...
- **www-host collapse on upsert.** `upsert_startup` is idempotent on exact `website` **and** on `_site_key` (www stripped + aliases; monorepo = host/owner/repo). Prevents EvenUp/Poolside-style double SIDs from `example.com` vs `www.example.com`.
- **Select gate rejects MVP stubs.** `decisions.is_template_wedge_description` + `rank_wedges_by_fit` penalize empty/`MVP:`/`MVP angle:` stubs (×0.25) and prefer non-template + non-thin evidence for selected≥1. Strip leading `MVP:` labels on substantive bodies via `strip_mvp_template_prefix`.
