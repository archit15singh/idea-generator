# 05 / Select + validate (gate before build)

Pick the top wedge per startup by fit; **verify the wedge exists for a real prospect before any MVP gets built**. This stage is the factory's honesty check (raw `structural-problems.md`).
...
- evidence tightness × 0.4 (a wedge with no `startup_competitive`/`startup_customer` citation gets evidence=0 — auto-disqualified)
...
2. Send to plausible economic buyers from the cold-reachable list in `raw/personalisation-and-founder-history.md`.
...
| ≥3 replies indicating real pain within 8 weeks | kill metric sustained (raw `analysis.md`); factory keeps running |
...
- raw `structural-problems.md`, `raw/factory.md`, `raw/analysis.md`