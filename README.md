...
| Table | Count | Notes |
|-------|-------|-------|
| `startups` | 152 | **152 scored** (all stage_marker=scored) |
| `analysed` (cohort) | 152 | full pool analysed |
...
| `candidate_startups` | 277 | pending_ingest≈40 |
...
**`plan_recursive_fanout` next_action = `ingest`**. Analyse-21 E2E done: Robust Intelligence (#156 Better evaluation), Nightfall (#157 AI-native), Abridge (#158 Better evaluation), Parabola (#159 Better memory), LiteLLM (#160 Open source). Primary mix Better memory 46, Better evaluation 44, Developer-first 22, AI-native 21.
...
- **Done (pushed):** CANONICAL **27/27** analysed; e2e **148/152** full (4 thin-evidence legacy: Doppler/Lattice/Unit21/Lovable); wedges 3040; personal_fit 152; **152 primary**; **patterns 18**; startups 152. Latest: **analyse-21** — Robust/Nightfall/Abridge/Parabola/LiteLLM full E2E (path+wedges+infra+fit+shortlist). next **ingest**. **88 tests green.**
- **Next fire:** `ingest` wave (Protect AI, Railway, Egress, Sierra, Artisan) then analyse drain.
...
1. **Ingest** next batch (≤5) then analyse→score→select immediately.
